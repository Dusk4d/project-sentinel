...
同一命令还会在每轮完整工具调用后原子保存 `agent-checkpoint.json`。模型服务中断时，再次提交完全相同的任务即可从最后完成轮次继续，不会重复该轮工具调用；其他任务不能覆盖未完成检查点。成功完成后检查点改为完成状态，新任务随后可以正常开始。检查点同样限制为 1 MiB，并可能包含受限工具输出，因此状态目录必须妥善保护。
...
使用 `--state-status` 可安全判断 `RUNNING` 或 `IDLE`；`IDLE` 时显示的元数据代表上一次持锁任务，不表示该 PID 仍在运行。该命令还显示 Agent 检查点为 `NONE`、`ACTIVE` 或 `COMPLETED`；活动检查点会列出绑定工作区、原任务和已完成轮次，便于准确恢复而不是覆盖状态。
