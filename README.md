...
本地 RAG 不依赖向量数据库或云端 embedding：它对受限文本文件分块，为英文词和中文字符/双字词建立内存索引，使用 BM25 风格评分返回最多五条证据。Web 会按真实路径、文件大小和修改时间复用未变化文件的分块，只重建新增或变化文件，并立即移除已删除文件；替换 ZIP 时清除旧项目缓存。对“项目是什么、主要功能、怎么启动、架构、技术栈、测试”等常见意图，会过滤低信息词并优先 README、架构文档、构建清单或测试代码，同时降低面试稿、报告和临时生成内容对概括问题的干扰；同文件重叠片段会去重。每条证据包含相对路径、起止行和相关度；回答为清理展示标记后的保守抽取式摘要，并明确要求结合来源核验。JSON 同时返回实际索引文件数、分块数和截断状态；确认存在超出安全上限的可索引内容时，抽取回答与模型提示都会声明结果可能不完整。`.env`、凭据、私钥、构建输出、依赖目录、超大文件和越界符号链接不会进入索引。该能力是真实的检索增强，但不等同于已经接入生成式大模型。

概括类问题若额外点名具体功能或模块，本地检索还要求该限定词在证据中有足够匹配；意图扩展和 README 排序加分不能代替这类证据。未命中时会明确说明缺少足以支持回答的本地证据，而不是给出无关摘要。
...
`--serve` 启动后访问 `http://127.0.0.1:8787/`。服务在启动工作区向下四层发现项目，并在页面提供下拉选择；也可以点击“上传 ZIP 检测”选择本机压缩包。ZIP 只发送到本机回环服务，解压到启动工作区内的临时目录；检测成功后页面自动选择该临时项目，因此 RAG、模型增强问答、Function Calling Agent、重新扫描和 JSON 下载都可继续使用。上传另一份 ZIP 或关闭服务时会删除临时副本；ZIP Agent 使用无状态模式，不把临时项目写入持久记忆。上传限制为压缩包 20 MiB、解压总量 100 MiB、单文件 10 MiB 和 5000 个条目，并拒绝路径穿越。没有识别到构建清单或 Git 根时，将工作区本身作为单项目。服务只绑定本机回环地址，不接受其他电脑连接。`/api/projects` 返回本地项目及当前临时 ZIP 的受限项目目录，常规分析接口只接受其中的不透明 ID，不能用客户端路径越过启动工作区。页面调用 `/api/analysis?project=<id>`，上传则调用只接受 `application/zip` 的 `/api/upload-analysis`；后端都只扫描一次并同时返回 `report` 与 `plan`，保证健康分、发现和优先行动来自同一时刻。兼容接口 `/api/report` 继续保留。页面展示行动排名、理由、预计恢复分和理论目标分。接口只执行静态分析，不执行被扫描项目；`/api/health` 可用于确认后端存活。按 `Ctrl+C` 停止服务。页面响应明确声明 UTF-8，因此不受 PowerShell 代码页影响。

若提供第四个参数作为 Agent 状态目录，Web 页面会为每个已发现项目使用独立子目录，并启用与 `--agent-ai-memory` 相同的持久记忆、进程锁和中断检查点；未提供时服务保持无状态且不会创建状态目录。状态目录应位于启动工作区之外并限制访问权限。页面显示当前项目的记忆条数与 `NONE`、`ACTIVE`、`READY` 或 `COMPLETED` 检查点状态；只读 `/api/agent-state` 接口不返回状态路径、任务文本或回答内容。
...
Agent 的内容工具不会读取 `.env`、凭据/私钥文件以及 `.git`、`target`、`node_modules`、`build`、`dist` 等常见生成目录。`search` 最多扫描 2000 个符合条件的文件，每条命中最多返回围绕关键词的 500 字符摘录，总输出不超过 64 KiB；只有确认存在超出文件或命中上限的内容时才标记结果不完整。该策略按名称降低意外外发风险，但不能识别普通源码中硬编码的所有秘密；外部模型仍应只用于允许共享的项目。

所有模型请求在联网前按 UTF-8 字节计算并限制为 1 MiB，模型响应限制为 2 MiB。Agent 写入模型上下文的每次工具输出还会按完整 Unicode 字符安全截断到 16 KiB，并附加显式截断标记；即使连续执行 20 次大文件工具调用，也会为系统提示、工具定义和模型消息保留请求余量。
...
同一命令还会在每轮完整工具调用后原子保存 `agent-checkpoint.json`。模型服务中断时，再次提交完全相同的任务即可从最后完成轮次继续，不会重复该轮工具调用；其他任务不能覆盖未完成检查点。模型回答完成后先进入 `READY` 状态并保存最终结果，只有记忆写入成功才改为 `COMPLETED`；提交窗口故障时重试会复用最终结果，不会再次调用模型。检查点同样限制为 1 MiB，并可能包含受限工具输出，因此状态目录必须妥善保护。
...
使用 `--state-status` 可安全判断 `RUNNING` 或 `IDLE`；`IDLE` 时显示的元数据代表上一次持锁任务，不表示该 PID 仍在运行。该命令还显示 Agent 检查点为 `NONE`、`ACTIVE`、`READY` 或 `COMPLETED`；未完成检查点会列出绑定工作区、原任务和已完成轮次，便于准确恢复而不是覆盖状态。
