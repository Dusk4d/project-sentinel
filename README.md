...
同一模型配置还会启用页面中的“Agent 分析”按钮和 `POST /api/agent-ai?project=<项目ID>`。请求体是 UTF-8 `text/plain` 任务；后端让模型从五个只读工具中自主选择，并返回 `answer`、`modelRounds`、`toolCalls` 和按执行顺序排列的 `trace`。页面展示工具名、受 160 字符限制并移除控制字符的调用参数摘要及成功状态，不重复返回可能敏感的工具输出。该接口使用项目白名单、8 KiB 请求上限和共享扫描准入门禁；与 RAG 不同，模型或工具循环失败会明确返回错误，不会伪装成已完成。
