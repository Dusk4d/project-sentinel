...
Agent 的内容工具不会读取 `.env`、凭据/私钥文件以及 `.git`、`target`、`node_modules`、`build`、`dist` 等常见生成目录。`search` 的每条命中最多返回围绕关键词的 500 字符摘录，总输出不超过 64 KiB，并在截断时显式说明。该策略按名称降低意外外发风险，但不能识别普通源码中硬编码的所有秘密；外部模型仍应只用于允许共享的项目。
