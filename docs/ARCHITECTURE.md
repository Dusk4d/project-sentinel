...
`ToolCallingAgentService` 实现完整的 Chat Completions 工具调用循环。它发送 `FunctionRegistry` 的工具 Schema，解析 assistant `tool_calls`，严格校验 `{input: string}` 参数，通过 `WorkspaceAgent` 调度，并把受限工具结果作为 tool 消息加入下一轮。客户端重建标准 assistant 工具消息并移除响应专用 `index`，发送前也规范化旧检查点，兼容 Ollama 对整数索引的严格校验。通用 `JsonCodec` 负责结构化解析，不依赖字段顺序或字符串搜索。模型轮次、单轮及总调用数均有硬上限。
