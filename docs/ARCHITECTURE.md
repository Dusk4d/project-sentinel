...
`LocalRagService` 构建有界内存索引。文本按 10 行分块、2 行重叠，英文按词、中文按字符和双字词切分，再以 BM25 风格相关度排序。长生命周期 Web 项目按真实路径、字节数和最后修改时间复用未变化文件的分块与词元；新增或变化文件局部重建，删除文件淘汰，ZIP 替换和服务关闭会清理对应缓存。每次查询仍遍历受限文件集合以保证变更可见。结果是带相对路径和行号的证据集合；`RagAnswer` 只抽取最高相关片段，不把检索内容解释为可执行指令。索引文件数、块数和单文件大小均有硬上限。
